...
- Finally need to stop, start nginx and check it’s status.If it is running properly,then will not see any issue in **nginx status**.Now need to hit in web browser DNS name of the web node on **port number 8484** and HTTP request  should redirect to one of the application node.In my case able to load-balance HTTP request to one of the app node in a round-robin manner.
